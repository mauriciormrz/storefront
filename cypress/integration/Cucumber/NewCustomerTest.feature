Feature: Place an order for the first time with a new customer
  ...
  @Integration
  Scenario Outline: New customer placing a one-time orderfor the first time
    Given I am a new user with "<first_name>", "<last_name>", "<phone_number>" and "<password>"
    ...
    And I add a new credit card
      | first_name | last_name  | card_number      | month | year | cvv | default_payment | billing_addres |
      | YL         | Visa       | 4440000009900010 | 01    | 2023 | 111 | Yes             | Yes            |
      | YL         | Mastercard | 5500005555555559 | 01    | 2023 | 111 | Yes             | Yes            |

    When I place a one-time first order with "<shipping_method>", "<payment_method>", "<donation>" and "<referral>"
    ...
    Examples:
      | first_name | last_name | phone_number | password  | shipping_method            | payment_method   | donation | referral |
